...
import (
	...
	"github.com/ti-mo/conntracct/pkg/epoll"
...
func cpuForEvent(event *utils.EpollEvent) int {
	return int(event.Pad)
}

type RecordUnmarshal interface {
	Unmarshal(buf []byte) int
}
...
func readLostRecords(rd io.Reader) (uint64, error) {
	...
	err := binary.Read(rd, utils.NativeEndian, &lostHeader)
	if err != nil {
...
func readRawSample(rd io.Reader) ([]byte, error) {
	...
	var size uint32
	if err := binary.Read(rd, utils.NativeEndian, &size); err != nil {
...
func NewReaderWithOptions(array *ebpf.Map, perCPUBuffer int, opts ReaderOptions) (pr *Reader, err error) {
	...
	pr = &Reader{
		...
		rings:       rings,
		poller:      poller,
		epollEvents: make([]utils.EpollEvent, len(rings)),
...
func (pr *Reader) PutTail(ring interface{}) {
	if ring == nil {
		return
	}

	evRing, _ := ring.(*perfEventRing)
	if evRing != nil {
		evRing.writeTail()
	}
}
